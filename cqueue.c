...
void mcu_util_cqueue_enque(cqueue_s *s, void *source)
{	
	...
	s->pos_tail = (s->pos_tail + 1) % s->size;
	...
	if (s->pos_head == s->pos_tail)
	{
		...
		s->pos_head = (s->pos_head + 1) % s->size;
	}
}

void mcu_util_cqueue_deque(cqueue_s *s, void *target)
{
